...
class SubtitleSegment {
  ...
  factory SubtitleSegment.fromJson(Map<String, dynamic> json) {
    return SubtitleSegment(
      ...
      startTime: double.tryParse(json['start_time']?.toString() ?? '0') ?? 0.0,
      endTime: double.tryParse(json['end_time']?.toString() ?? '0') ?? 0.0,
    );
  }
...
class SubtitleParser {
  ...
  static List<SubtitleSegment> parseFromAws(String jsonString) {
    try {
      final data = jsonDecode(jsonString);

      // Access the 'results' field. It should be a Map.
      final results = data['results'] as Map<String, dynamic>?;

      // Check if 'results' is present and has 'audio_segments'.
      final segments = results?['audio_segments'];

      if (segments is! List) {
        // Handle case where 'audio_segments' is missing or not a List
        print('AWS parser: audio_segments not found or not a list.');
        return [];
      }

      // Map the list of segment maps to SubtitleSegment objects.
      // Use Map<String, dynamic>.from to ensure type safety for the Map.
      return segments.map((seg) {
        // Ensure 'seg' is treated as a Map for SubtitleSegment.fromJson
        return SubtitleSegment.fromJson(Map<String, dynamic>.from(seg as Map));
      }).toList();
    } catch (e, st) {
      print('Error parsing AWS subtitles: $e\n$st');
      return [];
    }
  }
    ...
    try {
      ...
      switch (widget.format) {
        case SubtitleFormat.json:
          // Try normal parser first, fallback to AWS format
          _subtitles = SubtitleParser.parseFromJson(widget.subtitleData);
          if (_subtitles.isEmpty) {
            _subtitles = SubtitleParser.parseFromAws(widget.subtitleData);
          }
          break;
